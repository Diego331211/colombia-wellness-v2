...
export default function SponsorshipTiers() {
  ...
  const packages = t.raw("packages") as Record<string, { name: string; price: string; values: string[] }>;
  ...
  const getCardStyle = (key: string) => {
    ...
    return styles[key as keyof typeof styles] || "bg-white border border-gray-200";
  };
  ...
  const getButtonStyle = (key: string) => {
    ...
    return styles[key as keyof typeof styles] || "bg-slate-700 hover:bg-slate-800 text-white";
  };
          ...
          >
            <div className="flex gap-4 pb-4" style={{ width: 'max-content' }}>
              {packageKeys.map((key, packageIndex) => (
                <motion.div
                  key={key}
                  className={`rounded-lg shadow-sm min-w-[280px] max-w-[280px] flex flex-col ${getCardStyle(key)}`}
                ...
                >
                  {/* Card Header */}
                  <div className={`${getHeaderStyle(key)} p-4 text-center rounded-t-lg flex-shrink-0`}>
                  ...
                  <div className="p-4 flex-grow">
                    <div className="space-y-3">
                      {features.map((feature, featureIndex) => {
                        ...
                        if (value === "✓") {
                          return (
                            <div key={featureIndex} className="flex items-start gap-3 py-1.5">
                              {renderCell(value)}
                  ...
                  <div className="p-4 flex-shrink-0 border-t border-gray-200">
                    <MyButton
                      ...
                      className={`w-full py-2.5 text-sm rounded-lg font-medium transition-colors ${getButtonStyle(key)}`}
                    >
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  ...
                  {packageKeys.map((key) => (
                    <th key={key} className="border border-gray-400 p-4 text-center font-bold bg-gray-100 text-sm min-w-[140px]">
              ...
              <tbody>
                {features.map((feature, featureIndex) => (
                  ...
                  >
                    ...
                    {packageKeys.map((key) => (
                      <td key={key} className="border border-gray-400 p-4 text-center bg-white/30">
